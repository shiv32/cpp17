...
double Spreadsheet::Cell::stringToDouble(std::string_view inString)
{
  return strtod(inString.data(), nullptr);
}

size_t Spreadsheet::Cell::getTotalNumOfAccess() const
{
  return mNumAccesses;
}

std::string Spreadsheet::Cell::doubleToString(double inValue)
{
...
Spreadsheet::Spreadsheet(size_t width, size_t height, std::string sheetname, const SpreadsheetApplication &theApp)
    : mId(sCounter++), mWidth(std::min(width, kMaxWidth)) // std::min() requires <algorithm>
      ,
      mHeight(std::min(height, kMaxHeight)), sheetName(sheetname), mCells(mWidth, std::vector<std::shared_ptr<Spreadsheet::Cell>>(mHeight)), mTheApp(theApp)
{
    std::cout << __PRETTY_FUNCTION__ << std::endl;

    //----------------- default value at cell (1,1) ----------------------
    auto sc1 = std::make_shared<Spreadsheet::Cell>(8.98);
...
void Spreadsheet::setCellAt(size_t x, size_t y, const std::shared_ptr<Spreadsheet::Cell> &cell)
{
    /*
...
std::shared_ptr<Spreadsheet::Cell> &Spreadsheet::getCellAt(size_t x, size_t y)
{
    std::cout << __PRETTY_FUNCTION__ << std::endl;
...
{
    return mTheApp.ConstDefaultSheetName();

    // return mTheApp.DefaultSheetName(); //compiler error for non-constant
}
