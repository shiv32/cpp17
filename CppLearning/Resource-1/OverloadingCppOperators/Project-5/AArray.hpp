// create an associative array in which you use string keys instead of integers.
...
private:
    ...
    {
    public:
        Element(std::string_view key, const T &value): mKey(key), mValue(value) {}
        std::string mKey;
        T mValue;
    };
...
const T &AssociativeArray<T>::operator[](std::string_view key) const
{
    ...
    using namespace std::string_literals;//inside the function or file.
    throw std::invalid_argument("Key '"s + key.data() + "' doesn't exist");//note std::string_literals::operator""s
}