...
// read all CPU lines from /proc/stat
...
int main() {
    ...
    while (true) {
        ...
        // Per-thread usage compact (4 per line)
        {
            vector<string> cpuThreadStrings;
            for (size_t i = 1; i < nowCpuAll.size(); ++i) {
                double u = calcCpuUsage(prevCpuAll[i], nowCpuAll[i]);
                ostringstream th;
                th << "CPU" << (i-1) << ": " << fixed << setprecision(2) << u << " %";
                cpuThreadStrings.push_back(th.str());
            }
            prevCpuAll = nowCpuAll;

            for (size_t i = 0; i < cpuThreadStrings.size(); i += 4) {
                string line;
                for (size_t j = i; j < i+4 && j < cpuThreadStrings.size(); ++j) {
                    line += cpuThreadStrings[j];
                    if (j+1 < i+4 && j+1 < cpuThreadStrings.size())
                        line += "   ";
                }
