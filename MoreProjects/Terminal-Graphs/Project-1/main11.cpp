               ...
               int maxSec, int maxBandwidth)
{
    ...
    // Plot: newest data is on the left
...
int main() {
    ...
    while (true) {
        // Insert new samples at the front
        rxData.insert(rxData.begin(), dist(rng));
        txData.insert(txData.begin(), dist(rng));
        if ((int)rxData.size() > maxSec) rxData.pop_back();
        if ((int)txData.size() > maxSec) txData.pop_back();

        // Clear screen and redraw fresh frame
        clearAndHome();
